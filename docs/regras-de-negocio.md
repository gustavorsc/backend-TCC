...
| **RN14** | Ranking semanal (seg–dom) | `obterRankingSemanal`: soma `Tarefa.xpConcedido` com `dataConclusao` na semana corrente (fuso de São Paulo), agrupado por `Rotina.usuarioId`, ordenado desc. Não é tabela. | `services/ranking.service.ts`, `utils/tempo.ts` |
